-- | This is the identity list module that came with the space invaders source.
...
insertIL
  ...
  -> IL a
  -> (ILKey, IL a)
insertIL a IL {ilNextKey = k, ilAssocs = kas} =
  (k, il')
  where
...
insertILA_
  ...
  -> IL a
  -> IL a
insertILA_ f IL {ilNextKey = k, ilAssocs = kas} =
  il'
  where
...
listToILA as = IL
  { ilNextKey = length as
  , ilAssocs = reverse (zip [0..] (appFunc as [0..] ))
  } -- Maintain invariant!
...
deleteIL
  ...
  -> IL a
  -> IL a
deleteIL k IL {ilNextKey = nk, ilAssocs = kas} = IL
  { ilNextKey = nk
  , ilAssocs = deleteHlp kas
  }
  where
    deleteHlp []  = []
...
mapIL
  ...
  -> IL a
  -> IL b
mapIL f IL {ilNextKey = nk, ilAssocs = kas} = IL
  { ilNextKey = nk
  , ilAssocs = [(k, f ka) | ka@(k, _) <- kas]
  }
...
filterIL
  ...
  -> IL a
  -> IL a
filterIL p IL {ilNextKey = nk, ilAssocs = kas} = IL
  { ilNextKey = nk
  , ilAssocs = filter p kas
  }
...
mapFilterIL
  ...
  -> IL a
  -> IL b
mapFilterIL p IL {ilNextKey = nk, ilAssocs = kas} = IL
  { ilNextKey = nk
