...
findLeaf (x,y,z) (Branch node left right) = do
   ...
   if dstnc >= 0 then
              findLeaf (x,y,z) left else
              findLeaf (x,y,z) right
...
renderLeaves  frstm bitSet func mp leaf  = do
   ...
   Control.Monad.when clusterVisible (Control.Monad.when (boxInFrustum frstm (leafMin leaf) (leafMax leaf)) $ renderFaces bitSet mp (leafFaces leaf))
...
constructTree nodes lvs ind =
   if ind >= 0 then (do
      ...
      return (Branch currentNode leftNode rightNode)) else (do
...
adjustRGB lightMap factor offst = do
   ...
   let byteg2 = fromIntegral (truncate (g2 * tempb * 255.0) :: Int)
   let byteb2 = fromIntegral (truncate (b2 * tempb * 255.0) :: Int)
   pokeArray (advancePtr lightMap (3*offst)) [byter2,byteg2,byteb2]

scaleRGB :: Float -> Float -> IO (Float,Float)
scaleRGB clr scl = if clr > 1.0 && ((1.0/clr) < scl)
...
getAndPeek handle buf be =
  do bytesRead <- hGetBuf handle buf (sizeOf be)
