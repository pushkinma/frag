...
game
  :: BSPMap
  -> [ILKey -> Object]
  -> SF GameInput [ObsObjState]
game bspmap objs =
  loop
    (game' (case listToILA objs of x -> x) bspmap
      >>> arr (\oos -> (oos, oos)))
    >>>
    arr (\oos -> case map ooObsObjState (elemsIL oos) of y -> y)
...
game' objs bspmap =
  dpSwitch (route bspmap) objs (noEvent --> arr killOrSpawn) (\sfs' f -> game' (f sfs') bspmap)
...
route bspmap (gi, oos) = mapIL routeAux
  where
    ...
    messages = concatMap eventToList (elemsIL $ fmap ooSendMessage oos)
...
killOrSpawn (_, oos) =
  case foldl (mergeBy (.)) noEvent es of y -> y
  where
    es :: [Event (IL Object -> IL Object)]
    es =
      case
        [mergeBy (.) (ooKillReq oo `tag` deleteIL k) (fmap (foldl (.) id . map insertILA_) (ooSpawnReq oo))
          | (k, oo) <- assocsIL oos]
      of
        x -> x
...
clip mp gi
  ...
  ooses k msgs =
    ...
    in ObjInput
      { oiHit = listToEvent $ findCollidingObjects ooses (k, acube)
      ...
      , oiVisibleObjs =
          if h == 100 && t == (0, 0, 0)
            then listToEvent $ findVisibleTargets mp ooses (k, acube) 600
            else listToEvent $ findVisibleTargets mp ooses (k, acube) 1800
      , oiGameInput = gi
      }
clip mp gi
  OOSRay
...
checkCollision obj1 obj2
  ...
  | isAICube obj1 && isRay obj2 =
    rayBox (rayStart obj2) (rayEnd obj2) (x - cx, y - cy, z - cz) (x + cx, y + cy, z + cz)
    where
      (x, y, z) = oosOldCubePos obj1
...
checkVisible bsp obj1 obj2 range
  | isAICube obj1 && isCamera obj2 =
    aiVisTest bsp (x, y, z) (oosCubeAngle obj1) (cx, cy, cz) range
    where
      ( x,  y,  z) = oosOldCubePos obj1
...
findMessages key messages =
  listToEvent $ map snd (filter (\(destkey, _) -> destkey == key) messages)
