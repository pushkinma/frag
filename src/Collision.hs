...
clipObject mp pos oldpos (sx, sy, sz) =
  (colPos, ground)
  where
    (colPos, _, ground, _) = traceStep (createBox (-sx, -sy, -sz) (sx, sy, sz)) mp oldpos pos
...
-- | Returns the point where the collision occurred, no sliding or stepping.
traceo
  :: CollisionType
  -> BSPMap
  -> Vec3
...
split cType cState startDist endDist startRatio endRatio start end (Branch node left right)
  | startDist < endDist =
    let result1 = checkNode cType cState right startRatio (middleR r1) start (middleV r1)
        result2 = checkNode cType cState left (middleR r2) endRatio (middleV r2) end
    in brushCompare result1 result2
  | startDist > endDist =
    let result1 = checkNode cType cState left startRatio (middleR r2) start (middleV r2)
        result2 = checkNode cType cState right (middleR r1) endRatio (middleV r1) end
    in brushCompare result1 result2
  | otherwise =
    let result1 = checkNode cType cState left startRatio (middleR 1.0) start (middleV 1.0)
        result2 = checkNode cType cState right (middleR 0.0) endRatio (middleV 0.0) end
    in brushCompare result1 result2
  where
