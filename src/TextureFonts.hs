...
buildFonts = do
  ...
  let cxcys = [(realToFrac (x `mod` 16) / 16, realToFrac (x `div` 16) / 16) | x <- [0..(255 :: Int)]]
  for_ (zip cxcys lists2) genFontList
...
buildBigNums = do
  ...
  for_ (zip texs lists) genBigNumList
  return $ head lists
...
printFonts' x y (fontTex, DisplayList _) st string =
  unsafePreservingMatrix $ do
    ...
    alphaFunc $= Just (Greater, 0.1:: Float)
    for_ lists callList --(map DisplayList [17..(32::GLuint)])
    alphaFunc $= Nothing
    texture Texture2D $= Disabled

-- | Sets up the orthographic mode so we can draw at 2D screen coordinates.
setUpOrtho
  :: IO a
  -> IO ()
setUpOrtho func = do
