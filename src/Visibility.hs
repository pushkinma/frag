...
-- | Test if the object lies within the field of view.
fieldTest
...
rayTest bsp (x, y, z) vec2@(_, _, _) =
  not (snd $ clipRay2 bsp vec2 (x, y, z) (0, 0, 0))
...
clipRay2 mp pos oldpos (_, _, _) =
  (colPos, collided)
  where
    (colPos, collided, _, _) = traceo (createSphere 0) mp oldpos pos
...
traceo cType mp start end
  | newRatio /= 1.0 = (vectorAdd start (vectorMult (vectorSub end start) newRatio), hasCol, step, grounded)
...
checkBrush' start end cType brush
  | (numOfBrushSides brush > 0) && (textureType brush == 1) =
    let colout = checkBrushSides start end cType False False False False (-1.0) 1.0 (0, 0, 0) (brushSides brush)
    in case colout of
...
checkBrushSides  (_, _, _) (_, _, _) _ out collided step ground startR endR cNorm [] =
  Just (out, collided, step, ground, startR, endR, cNorm)
checkBrushSides start@(x, _, z) end@(x1, _, z1) cType out collided step ground startR endR cNorm (b:bs)
  | startDist >  0 && endDist >  0 = Nothing
  | startDist <= 0 && endDist <= 0 = continue
  | startDist > endDist =
      if ratio1 > startR
        then checkBrushSides start end cType checkout True mayStep grounded ratio1 endR (bsPlaneNorm b) bs
        else continue
  | otherwise =
      if ratio2 < endR
        then checkBrushSides start end cType checkout collided step ground startR ratio2 cNorm bs
        else continue
  where
    checkout
      | startDist > 0 = True
    ...
    grounded
      | planey >= 0.2 = True
      | otherwise = ground
    planey    = snd3 (bsPlaneNorm b)
    ratio1    = (startDist - epsilon) / (startDist - endDist)
    ratio2    = (startDist + epsilon) / (startDist - endDist)
    continue  = checkBrushSides start end cType checkout collided step ground startR endR cNorm bs
...
checkNode cType cState (Branch node left right) startRatio endRatio start end =
  ...
  where
    ...
    recurse sDist eDist
      | sDist >= boffset && eDist >= boffset             = checkNode cType cState left  sDist eDist start end
      | sDist < ((-1) * boffset) && eDist < ((-1) * boffset) = checkNode cType cState right sDist eDist start end
      | otherwise = split cType cState sDist eDist startRatio endRatio start end (Branch node left right)
    ...
    in case result1 of
      ...
      _ ->
        ...
        in case result2 of
          ...
          _                       -> Nothing
  | startDist > endDist =
    let result1 = checkNode cType cState left startRatio (middleR r2) start (middleV r2)
    in case result1 of
      Just (True, _, _, _, _) -> result1
      _ ->
        let result2 = checkNode cType cState right (middleR r1) endRatio  (middleV r1) end
        in case result2 of
          Just (True, _, _, _, _) -> result2
          _                       -> Nothing
  | otherwise =
    let result1 = checkNode cType cState left startRatio (middleR 1.0) start (middleV 1.0)
    in case result1 of
      Just (True, _, _, _, _) -> result1
      _ ->
        let result2 = checkNode cType cState right (middleR 0.0) endRatio  (middleV 0.0) end
        in case result2 of
          Just (True, _, _, _, _) -> result2
          _                       -> Nothing
    where
