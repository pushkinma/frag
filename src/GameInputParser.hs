...
-- done better in the new FRP.Yampa framework.
module GameInputParser (
  WinInput,
  GameInput,     -- Abstract
...
parseWinInput =
  ...
  (wiToPDS <<< arr snd) &&&
  (wiToKs  <<< arr fst) &&&
  gDt >>^ \((cmdStr, cmd), (pds, (ks, (dt, t)))) -> GameInput
    { giCmdStr = cmdStr
    , giCmd    = cmd
    , giPDS    = pds
    , giKS     = ks
...
dragStop :: SF GameInput (Event Distance2)
dragStop =
  ((giPDS >>> pdsDrag) ^>> edgeBy detectStop Nothing) &&& dragVec >>^ uncurry tag
  where
    ...
    scHlp pfx sfxconts = \case
      ...
      '.' ->
        case sfxconts of
          ...
          _             -> emitPfx (S (scHlp pfx' sfxconts')) pfx'
            where
