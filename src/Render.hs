...
renderHud gd playerState noos tme =
  setUpOrtho $ do
    ...
    printFonts' 0 464 (fonts gd) 1 $ "framerate = " ++ show (truncate ((1 / dt) :: Double) :: Int)

    --print the player's score
    printFonts' 0 448 (fonts gd) 1 ("ezpks = " ++show (score playerState))

    --print the position of the player
    let (q, w, e) = cpos (oldCam playerState)
    printFonts' 0 432 (fonts gd) 1 ("position = " ++show (truncate q :: Int, truncate w :: Int, truncate e :: Int))
    ...
    when (score playerState == nems gd && nems gd > 0) $
...
printLife font life
  ...
  | otherwise   = printf "(^_^)"
  where
    printf = printFonts' 292 32 font 1
...
renderRay OOSRay
  ...
  when cl $ do
    ...
    unsafePreservingMatrix $ do
      ...
      renderQuadric (QuadricStyle Nothing NoTextureCoordinates Outside FillStyle) (Sphere 3 12 12)
    cullFace $= Just Front
  color $ Color4 255 255 255 (255 :: GLubyte)
...
renderProjectile OOSProjectile {projectileOldPos = (x, y, z)} =
  unsafePreservingMatrix $ do
    ...
    renderQuadric (QuadricStyle Nothing NoTextureCoordinates Outside FillStyle) (Sphere 4 12 12)
    color $ Color4 50 50 160 (255 :: GLubyte)
    renderQuadric (QuadricStyle Nothing NoTextureCoordinates Outside FillStyle) (Sphere 3.5 12 12)
    color $ Color4 255 255 255 (255 :: GLubyte)
    renderQuadric (QuadricStyle Nothing NoTextureCoordinates Outside FillStyle) (Sphere 2.5 12 12)
    cullFace  $= Just Front
    depthFunc $= Just Less
...
renderEnemy
  ...
  -> ObsObjState
  -> IO ()
renderEnemy camRef mdels frust bspmap OOSAICube
  { oosOldCubePos = (x, y, z)
