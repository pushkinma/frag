...
data BSPPatch = BSPPatch
  { patchLOD    :: Int         -- ^ The level of tessellation
...
getControlPointIndices i width height =
  ...
  where
    create3x3ControlPoints x y =
      [i + ((y * 2 * width) + (x * 2)) + (row * width) + point | row <- [0..2], point <- [0..2]]
...
getControlPoints vertexData startIndex width height = do
  ...
  controlPoints <- for indcs $ readControlPoints vertexData
...
createPatches vertData startVert width height tesselation = do
  ...
  for controlPoints $ createPatch tesselation
...
createPatch''
  ...
  -> Array Int VertTup
  -> IO ()
createPatch'' tess ptr arr =
  for_ [1..tess] $ createPatch''' tess ptr arr
...
createPatch''' tess ptr arr u = do
  let tup1 = bezier tess (arr!0) (arr!1) (arr!2) u
      tup2 = bezier tess (arr!3) (arr!4) (arr!5) u
      tup3 = bezier tess (arr!6) (arr!7) (arr!8) u
      patchVerts = map (bezier tess tup1 tup2 tup3) [0..tess]
...
generateIndices
  :: Int
  -> IO (Ptr GLsizei, Ptr (Ptr GLint))
generateIndices tess = do
  indexArray <- newArray (0, (tess * (tess + 1) * 2) - 1) 0
  let pt1 = [((((row * (tess + 1)) + point) * 2) + 1, fromIntegral ((row * (tess + 1)) + point)) |
                 row <- [0..(tess - 1)],
                 point <- [0..tess]]
      pt2 = [(((row * (tess + 1)) + point) * 2, fromIntegral (((row + 1) * (tess + 1)) + point)) |
                row <- [0..(tess - 1)],
                point <- [0..tess]]
  for_ pt1 $ writeIndices indexArray
  for_ pt2 $ writeIndices indexArray
