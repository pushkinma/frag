...
createAWindow windowName level = do
  ...
  animList   <- for listModels getAnims
  ...
  idleCallback          $= Just (idle lasttime (newInput, newMouseInput) hasReact (tex, base) inpState rh)
  where
    getAnims (x, y) = do
...
render gd oos = do
  ...
  void $ readIORef (lastDrawTime2 gd)
  void $ readIORef (hasReacted gd)
  ...
  if True
    then do
      ...
      frust <- getFrustum
      for_ oos $ renderObjects (camera gd) (models gd) frust mp
...
getWinInput (lasttime, (newInput, newMouseInput)) inpState _ currenttime = do
  ...
  mmin <- case (coalesce newIn, coalesce newMouseIn, hasReset) of
...
coalesce (Just KBMInput {key = (MouseButton button), keyState = ks, pos = p }) =
  Event HGL.Button
    { HGL.pt     = pos2Point p
    , HGL.isLeft = isMBLeft (MouseButton button)
    , HGL.isDown = isKeyDown ks
    }
coalesce (Just KBMInput {key = (Char a), keyState = ks}) =
  Event HGL.Char
    { HGL.char = a
    , HGL.isDown = isKeyDown ks
    }
